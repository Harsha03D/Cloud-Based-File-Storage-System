...
function UploadPage() {
  ...
  const {
    primary_color, secondary_color, surface_color,
  ...
  const handleUpload = async () => {
    ...
    try {
      for (let f of selectedFiles) {
        ...
        // 2️⃣ Upload file directly to S3 via presigned URL
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}
      style={{ minHeight: "100vh", padding: "32px", position: "relative", overflow: "hidden" }}>
      
      {/* Floating icons */}
      <motion.div animate={{ y: [0, -20, 0] }} transition={{ duration: 6, repeat: Infinity }}
        style={{ position: "absolute", top: "10%", left: "6%", fontSize: "90px", opacity: 0.1 }}>
        ☁️
      </motion.div>
      ...
      <div style={{ position: "absolute", top: "20px", left: "20px", display: "flex", gap: "10px" }}>
        <motion.button whileHover={{ scale: 1.1 }} onClick={() => navigate(-1)}
          style={{
            background: "#fff", border: `2px solid ${primary_color}`,
      ...
      <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.7 }} style={{ textAlign: "center", marginTop: "80px", color: text_color }}>
        <h1 style={{ fontSize: "2.5rem", fontWeight: "700" }}>{config.page_title}</h1>
      ...
      <motion.div
        ...
        animate={{ boxShadow: isDragging ? `0 0 25px ${primary_color}50` : "0 10px 30px rgba(0,0,0,0.1)" }}
        transition={{ duration: 0.3 }}
        style={{
          margin: "50px auto",
          maxWidth: "700px",
          backgroundColor: surface_color,
          border: `3px dashed ${isDragging ? primary_color : text_color + "30"}`,
          borderRadius: "24px",
          padding: "60px 40px",
          textAlign: "center",
          cursor: "pointer",
      ...
      >
        <motion.div animate={{ rotate: [0, 5, -5, 0] }} transition={{ repeat: Infinity, duration: 6 }}
          style={{ fontSize: "80px", color: primary_color, marginBottom: "16px" }}>
          ☁️
        </motion.div>

        <h2 style={{ color: primary_color, fontWeight: 600 }}>{config.drop_zone_text}</h2>
        <p style={{ color: text_color, opacity: 0.7 }}>{config.browse_text}</p>

        <input ref={fileInputRef} type="file" multiple onChange={(e) => handleFileSelect(e.target.files)} style={{ display: "none" }} />
      </motion.div>
        ...
        {selectedFiles.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
            ...
            style={{
              ...
              backgroundColor: surface_color, borderRadius: "16px",
          ...
          >
            ...
            <motion.button whileHover={{ scale: 1.05 }} onClick={handleUpload}
              disabled={isUploading}
              style={{
                background: `linear-gradient(135deg, ${primary_color}, ${secondary_color})`,
                color: "white", border: "none", borderRadius: "12px",
                padding: "12px 24px", marginTop: "12px", cursor: "pointer",
                fontWeight: 600
              }}>
            ...
            <ul style={{ marginTop: "20px", listStyle: "none", padding: 0 }}>
              {selectedFiles.map((file) => (
                <motion.li key={file.id} initial={{ opacity: 0, x: -20 }}
